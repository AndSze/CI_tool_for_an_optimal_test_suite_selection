...
public class TcpClient {
	...
	public TcpClient(int serverPort, String message) throws UnknownHostException, IOException {
		...
		try {
			// it is a simple socket
			// first argument is ip address, the second one is port
			this.socket = new Socket(InetAddress.getLocalHost().getHostAddress(), this.serverPort);
			DataInputStream in = new DataInputStream(this.socket.getInputStream());
			DataOutputStream out = new DataOutputStream(this.socket.getOutputStream());
			//String data = in.readUTF(); // read a line of data from the stream
			out.writeUTF(this.message); // UTF is a string encoding
			}
		catch (UnknownHostException e) { System.out.println("Socket:" + e.getMessage()); }
		catch (EOFException e) { System.out.println("EOF:" + e.getMessage()); }
		catch (IOException e) { System.out.println("readline:" + e.getMessage()); }
