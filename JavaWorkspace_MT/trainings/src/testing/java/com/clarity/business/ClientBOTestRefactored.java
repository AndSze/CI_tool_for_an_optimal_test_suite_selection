...
// REMEMBER:
// 1) always test a single condition under one particular method !!!!!
// 2) jUnit test should fail only if the code fails - jUnit shouldn't be dependent on the external factors
...
public class ClientBOTestRefactored {
	...
	// name test unambiguously
	@Test
	public void testClientProductSum_AllProductsSameCurrency()
			throws DifferentCurrenciesException {

		// differentiation what is actual important in this test - highlight the important values only
		...
		// create a dedicated method outside the test for things that aren't important
		List<Product> products = createProductListWithAmounts(amounts);

		Amount actual = clientBO.getClientProductsSum(products);

		// create a dedicated method to compare rather than have two assertEquals()
