...
public class ReadMessageTest {
	...
	String[] testPurpose = { "Verify that once the readMessage() function is called, an incoming message from TCPserver is read from an input object stream",
   ...
   /***********************************************************************************************************
	 ...
	 * Description: 				Verify that once the readMessage() function is called, an incoming message from TCPserver is read from an input object stream
   ...
   /***********************************************************************************************************
	...
	public void test_run_3() throws IOException, InterruptedException {
		...
		testThread_exception = new Thread(new Runnable() {
			...
			public void run() {
				try {
					clientManager_1.readMessage(clientManager_1.getInputReaderStream());
				} catch (ClassNotFoundException e) {
					// To prove that exception's stack trace reported by JUnit caught ClassNotFoundException
					assertTrue(false);
					e.printStackTrace();
