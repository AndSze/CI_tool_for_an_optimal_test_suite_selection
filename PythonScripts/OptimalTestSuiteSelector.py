...
# Parse git diff log to separate file blocks (method body) for all changes (added/removed lines or added files)
...
print '4) Parse git diff log to separate file blocks (method body) for all changes (added/removed lines or added files) \n'
...
len_resulted_array_of_added_lines_in_changed_file_blocks = len(resulted_array_of_added_lines_in_changed_file_blocks)
for element in resulted_array_of_added_lines_in_changed_file_blocks:
	if (len(element) == 0):
		len_resulted_array_of_added_lines_in_changed_file_blocks = len_resulted_array_of_added_lines_in_changed_file_blocks - 1
		
print '5) Parse git diff log to separate file blocks (method body) for methods with added lines \n'
print '\t len(resulted_array_of_added_lines_in_changed_file_blocks): ' + str(len_resulted_array_of_added_lines_in_changed_file_blocks)
print '\n'
...
# do not include empty elements
len_resulted_array_of_removed_lines_in_changed_file_blocks = len(resulted_array_of_removed_lines_in_changed_file_blocks)
for element in resulted_array_of_removed_lines_in_changed_file_blocks:
	if (len(element) == 0):
		len_resulted_array_of_removed_lines_in_changed_file_blocks = len_resulted_array_of_removed_lines_in_changed_file_blocks - 1
		
print '6) Parse git diff log to separate file blocks (method body) for methods with removed lines \n'
print '\t len(resulted_array_of_removed_lines_in_changed_file_blocks): ' + str(len_resulted_array_of_removed_lines_in_changed_file_blocks)
print '\n'
...
print '7) Parse git diff log to separate file blocks (method body) for methods that have been added \n'
...
print '8) Parse file blocks (method body) for methods with added lines to separate names of the methods, as an output the normalized path to the file that contains the methods is also returned \n'
for methods_with_added_lines_numbers in resulted_array_of_methods_with_added_lines_numbers:
	if( len(methods_with_added_lines_numbers) != 0):
...
print '9) Parse file blocks (method body) for methods with removed lines to separate names of the methods, as an output the normalized path to the file that contains the methods is also returned \n'
for methods_with_removed_lines_numbers in resulted_array_of_methods_with_removed_lines_numbers:
	if( len(methods_with_removed_lines_numbers) != 0):
...
print '10) Parse file blocks (method body) for methods that have been added to separate names of the methods, as an output the normalized path to the file that contains the methods is also returned \n'
...
print '11) Concatenate the results from points 7., 8. and 9. (i.e. build single list of all methods that have been affected with normalized path to the file that contains the methods) \n'
for aggregated_affected_method in resulted_array_of_aggregated_affected_methods:
	if( len(aggregated_affected_method) != 0):
...
print '12) Create list of the methods affected by changes in newer commit against older commit \n'
for affected_method in resulted_list_of_affected_methods:
	print '\t affected method: ' + str(affected_method)
print '\n'
	
# Create lists of the methods that call the methods affected by changes in newer commit against older commit 
resulted_list_of_methods_that_calls_changed_internal_method, methods_that_calls_changed_internal_method_packages = build_resulted_list_of_methods_that_calls_internal_changed_method(resulted_array_of_affected_methods)
resulted_list_of_methods_that_calls_changed_external_method, methods_that_calls_changed_external_method_packages = build_resulted_list_of_methods_that_calls_external_changed_method(resulted_array_of_affected_methods)

iterator = 0
print '13) Create lists of the methods that call the methods affected by changes in newer commit against older commit \n'
for method_that_calls_affected_internal_method in resulted_list_of_methods_that_calls_changed_internal_method:
	print '\t method that calls affected internal method - package: ' + str(methods_that_calls_changed_internal_method_packages[iterator]) + ',\tmethod name: '+ str(method_that_calls_affected_internal_method)
	iterator += 1
iterator = 0
for method_that_calls_affected_external_method in resulted_list_of_methods_that_calls_changed_external_method:
	print '\t method that calls affected external method - package: ' + str(methods_that_calls_changed_external_method_packages[iterator]) + ',\tmethod name: '+ str(method_that_calls_affected_external_method)
	iterator += 1
print '\n'
...
resulted_optimal_unit_tests_suite = browse_for_an_optimal_unit_tests_suite(resulted_list_of_affected_methods, resulted_list_of_normalized_files_of_changed_methods, resulted_list_of_methods_that_calls_changed_internal_method, resulted_list_of_methods_that_calls_changed_external_method, methods_that_calls_changed_internal_method_packages, methods_that_calls_changed_external_method_packages)

print '14) Based on the lists of methods from points 11. and 12., create a list of unit tests that are optimal test suite for changes in newer commit against older commit (the list of tests is returned in format readable by pom.xml) \n'
...
print '15) Update pom.xml file with names of unit test to be executed\n'
